...
function ItemCardClients() {
  return (
    ...
    const { id, src, alt, heading, startDate, endDate, responsibilities, technologies } = ClientData;

    const calculateTenureDates = () => {   
        const getMonthNameStart = startDate.toLocaleString('default', {month: 'short'});
        const getMonthNameEnd = endDate.toLocaleString('default', {month: 'short'});
     ...
     const calculateTenureDifference = () => {
          const calculateYearsDifference = Math.floor(((endDate.getMonth() - startDate.getMonth() + 12 * (endDate.getFullYear() - startDate.getFullYear())) / 12));
          const calculateMonthsDifference = (endDate.getMonth() - startDate.getMonth() + 12 * (endDate.getFullYear() - startDate.getFullYear())) - (calculateYearsDifference * 12);
          const calculateYearString = calculateYearsDifference === 1 ? 'Year' : "Years";
          const calculateMonthString = calculateMonthsDifference === 1 ? 'Month' : "Months";
